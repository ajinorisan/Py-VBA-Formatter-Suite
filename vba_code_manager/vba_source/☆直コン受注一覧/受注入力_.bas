...
Sub 受注入力()
    ...
    With WS1.Sort
        .SetRange WS1.Range(Cells(1, 1), WS1.Cells(S1_A_LASTROW, S1_LASTCOLUMN_1))  '範囲をA1から右下方向は可変
        .Header = xlYes
        .MatchCase = False
    ...
    With WS1
        ...
        Dim S1_仕入金額_COL As Long    'シート1の1行目から"仕入金額($)"を探す
        S1_仕入金額_COL = WorksheetFunction.Match("仕入金額($)", .Rows("1:1"), 0)

        .Range(.Columns(S1_発注日_COL), .Columns(S1_仕入金額_COL)).EntireColumn.Hidden = True
        ...
        .Cells(S1_A_LASTROW + 1, 1).Activate
