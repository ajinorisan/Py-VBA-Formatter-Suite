...
Set WS7 = ThisWorkbook.Worksheets("前期")
 ...
 Dim ws As Worksheet, flag As Boolean
    ...
    For Each ws In Worksheets
        If ws.Name = "TEMP" Then flag = True
    Next ws
    If flag = False Then
...
.SumIfs(WS2.Range("F1:F" & WS2_AC_LR), WS2.Range("C1:C" & WS2_AC_LR), WS2.Cells(FOR1, 3), _
WS2.Range("E1:E" & WS2_AC_LR), WS2.Cells(FOR1, 5))
Next FOR1
...
WS5.Range(WS5.Cells(2, 3), WS5.Cells(WS5_AC_LR, 3)).Copy
WS2.Cells(WS2_CC_LR + 1, 3).PasteSpecial Paste:=xlPasteValues

WS2_CC_LR = WS2.Cells(WS2.Rows.Count, 3).End(xlUp).Row


If WS7.Cells(1, 1) <> "" Then
...
Set WS1 = ThisWorkbook.Worksheets("得意先対比")
 ...
 If WS1.AutoFilterMode = True Then
        WS1.Range("A2:U2").AutoFilter 'オートフィルタを解除
    End If
...
AWS1.Range("A2:U2").AutoFilter 20, "<>"
