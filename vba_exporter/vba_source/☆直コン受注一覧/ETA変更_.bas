...
Sub ETA変更()
    ...
    With WS1
        ...
        Dim S1_仕入単価_COL As Long    'シート1の1行目から"仕入単価"を探す
        S1_仕入単価_COL = WorksheetFunction.Match("仕入単価", .Rows("1:1"), 0)

        Dim S1_仕入金額_COL As Long    'シート1の1行目から"仕入金額($)"を探す
        S1_仕入金額_COL = WorksheetFunction.Match("仕入金額($)", .Rows("1:1"), 0)

        .Range(.Columns(S1_仕入単価_COL), .Columns(S1_仕入金額_COL)) _
        .EntireColumn.Hidden = True
