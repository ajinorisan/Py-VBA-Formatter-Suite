...
Sub 発注入力()
    ...
    With WS1
        ...
        S1_受注日付_COL = WorksheetFunction.Match("受注日付", .Rows("1:1"), 0)

        .Sort.SortFields.Add Key:=.Range _
        (.Cells(2, S1_受注日付_COL), .Cells(S1_A_LASTROW, S1_受注日付_COL)) _
        , SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
        ...
        .Sort.SortFields.Add Key:=.Range _
        (.Cells(2, S1_希望ETA_COL), .Cells(S1_A_LASTROW, S1_希望ETA_COL)) _
        , SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
        ...
        .Sort.SortFields.Add Key:=.Range _
        (.Cells(2, S1_希望輸入港_COL), .Cells(S1_A_LASTROW, S1_希望輸入港_COL)) _
        , SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    ...
    With WS1
        ...
        Dim S1_卸売金額_COL As Long    'シート1の1行目から"卸売金額"を探す
        S1_卸売金額_COL = WorksheetFunction.Match("卸売金額", .Rows("1:1"), 0)

        .Range(.Columns(S1_卸売単価_COL), .Columns(S1_卸売金額_COL)) _
        .EntireColumn.Hidden = True

        .Range(.Columns(S1_決定ETA_COL), .Columns(S1_決定輸入港_COL)).EntireColumn.Hidden = True

        .Range(.Columns(S1_INVOICENO_COL), .Columns(S1_済_COL)).EntireColumn.Hidden = True
        ...
        Dim S1_発注書No_LASTROW As Long    'シート1のA列の最後のデータ行取得
        S1_発注書No_LASTROW = .Cells(Rows.Count, S1_発注書No_COL).End(xlUp).Row

        .Cells(S1_発注書No_LASTROW + 1, 1).Activate
